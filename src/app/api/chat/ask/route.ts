...
Como consequência direta dessa simetria, todos os seus três ângulos internos também são iguais e medem exatamente 60 graus. É uma das formas mais estáveis e simétricas estudadas na geometria. Qual fórmula de área ou perímetro você gostaria de revisar agora?`;
...
O seno é a razão entre o cateto oposto e a hipotenusa; o cosseno é a razão entre o cateto adjacente e a hipotenusa; e a tangente é a divisão do cateto oposto pelo adjacente. Essas relações são aplicadas no cálculo de alturas de edifícios e trajetórias físicas. Qual desses conceitos está te gerando dúvidas?`;
...
Para resolver isso, usamos simplificações algébricas, fatoração clássica ou a regra de L'Hôpital para reescrever a função e encontrar o valor real do limite. Tem algum exercício específico que queira resolver?`;
...
Por exemplo, a derivada da posição em relação ao tempo nos dá a velocidade instantânea de um objeto. Graficamente, a derivada em um ponto representa a inclinação da reta tangente à curva naquele local. Qual regra de derivação está te travando?`;
...
A condicional se P, então Q só é classificada como Falsa no caso em que a primeira proposição é verdadeira e a segunda é falsa (a clássica regra da Vera Fischer). Em todas as outras combinações, ela é considerada Verdadeira. Qual operador lógico você gostaria de analisar?`;
    } else {
